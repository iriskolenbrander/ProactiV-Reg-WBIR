...
class SpatialTransformer(nn.Module):
    ...
    def warp_landmarks(self, src, flow):
        ...
        # idx = torch.round(src).to(torch.int64).squeeze()
        ...
        # NEW
        src_temp = torch.div(src, torch.tensor(flow.shape[2::]).to(flow.device)) * 2 - 1
        src_temp = src_temp.view(1, -1, 1, 1, 3).type(torch.float32)
        src_temp = src_temp.flip([-1])
        pred_xyz = F.grid_sample(flow, src_temp, mode='bilinear').squeeze().t()
        points_warped = src + pred_xyz.unsqueeze(0)
